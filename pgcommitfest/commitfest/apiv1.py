...
from .cfbot_queue import CfbotQueue, CfbotQueueItem
from .models import (
    CfbotBranch,
    CfbotBranchHistory,
    CfbotTask,
...
def fetch_open_patches(request):
    ...
    open_patches = (
        Patch.objects.filter(
            patchoncommitfest__status__in=PatchOnCommitFest.OPEN_STATUSES
        )
        .distinct()
        .order_by("id")
    )
    ...
    patch_list = [
        {
            ...
            "attachment_count": MailThreadAttachment.objects.filter(
                mailthread__patches=patch, messageid=patch.patchset_messageid
            ).count(),
            "patchset_messagedate": patch.patchset_messagedate,
            "in_queue": CfbotQueueItem.objects.filter(
                patch_id=patch.id
            ).exists(),  # New field
            "has_branch": CfbotBranch.objects.filter(
                patch_id=patch.id
            ).exists(),  # New field
        }
...
def add_test_data(request):
    ...
    from .fixtures.alpha import load
...
def enqueue_patch(request):
    ...
    if CfbotQueueItem.objects.filter(patch_id=patch_id).exists():
        return apiResponse(
            request, {"error": "Patch is already in the queue"}, status=400
        )

    queue.insert_item(patch_id=patch_id, message_id=patch.patchset_messageid)
    return apiResponse(
        request, {"success": True, "message": "Patch enqueued successfully."}
    )
...
def doCreateBranch(patch_id, message_id):
...
def create_branch(request):
    ...
    branch = doCreateBranch(patch_id, message_id)

    return apiResponse(
        request,
        {
            "branch_id": branch.branch_id,
            "message": f"Branch '{branch.branch_name}' created for patch_id {patch_id} with message_id {message_id}.",
        },
    )
...
def process_branch(request, branch_id):
    ...
    return apiResponse(
        request,
        {
            "message": f"Branch {new_branch.branch_name} has been created with status {new_branch.status}."
        },
    )
...
def cfbot_tasks(request):
    ...
    tasks = (
        CfbotTask.objects.filter(branch_id=branch_id).order_by("-modified")
        if branch_id
        else CfbotTask.objects.all()
    )
    task_list = [
        {
            "item_id": task.id,
...
def update_task_status(request, task_id):
    ...
    task.save()
    return apiResponse(
        request, {"message": f"Task {task_id} status updated to {new_status}."}
    )
...
def clear_branch_table(request):
    ...
    CfbotBranchHistory.objects.all().delete()
    return apiResponse(
        request, {"message": "Branch table and tasks cleared successfully."}
    )
...
def fetch_branch_history(request):
    ...
    history = CfbotBranchHistory.objects.filter(branch_id=branch_id).order_by(
        "-modified"
    )
    history_list = [
        {
...
def clear_branch_history(request):
    ...
    return apiResponse(
        request,
        {
            "message": f"Cleared {deleted_count} history entries for branch_id {branch_id}."
        },
    )
