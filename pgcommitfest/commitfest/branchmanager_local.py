...
import pgcommitfest.commitfest.workflow as workflow

from .models import CfbotTaskArtifact

def get_branch_manager(branch):
    return workflow.BranchManager(
...
class LocalPatchApplier(workflow.BranchManager.PatchApplierTemplate):
...
class LocalPatchCompiler(workflow.BranchManager.PatchCompilerTemplate):
    ...
    def get_delay(self, branch):
        return 60
class LocalPatchTester(workflow.BranchManager.PatchTesterTemplate):
    def __init__(self, working_dir, repo_dir):
        super().__init__()
        self.working_dir = working_dir
        self.repo_dir = repo_dir
...
def getNotifier():
    return workflow.BranchManager.Notifier()
